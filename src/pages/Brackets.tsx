...
const Brackets = () => {
  ...
  const MatchNode = ({ match, side }: { match: Match, side: 'left' | 'right' }) => {
    ...
    return (
      <motion.div 
        layout
        className={`relative w-48 sm:w-56 bg-white border-2 rounded-lg shadow-xl overflow-hidden transition-all duration-500 z-10 ${
          match.isFinished ? 'border-neutral-100' : 'border-brand ring-4 ring-brand/10'
        }`}
      >
        <div className={`px-3 py-1 flex justify-center items-center text-xs font-black uppercase ${
          match.isFinished ? 'bg-neutral-50 text-neutral-400' : 'bg-brand text-white'
        }`}>
          <span>{match.round}</span>
          {!match.isFinished && <div className="flex items-center gap-1.5 animate-pulse"><Zap size={10} fill="currentColor" /> VIVO</div>}
        </div>
        <div className="p-2 space-y-1.5 bg-white">
          <div className={`flex justify-between items-center px-3 py-2 rounded-xl transition-all ${
            isWinnerA ? 'bg-brand text-white shadow-lg' : 'bg-neutral-50 text-neutral-600'
          }`}>
          ...
          <div className={`flex justify-between items-center px-3 py-2 rounded-xl transition-all ${
            isWinnerB ? 'bg-brand text-white shadow-lg' : 'bg-neutral-50 text-neutral-600'
          }`}>
             <div className="min-w-0">
